...
export default function Uploader(props) {
  ...
  const [addPost] = useMutation(MUTATION);
    ...
    try {
      ...
      papaparse.parse(file, {
        ...
        //   console.log(tidyData);
          tidyData.forEach((element) => {
            onSubmitData(element);
          });
  ...
  const onSubmitData = (data) => {
    ...
    addPost({
      variables: {
        ...
        location_des: data.location.description,
      },
    });
  };
  ...
  const tableData = rawData.map((record, idx) => (
    <tr key={idx}>
      {headers.map((col) => (
        <td key={`${idx}_${col}`}>{record[col]}</td>
      ))}
    </tr>
  ));
  return (
