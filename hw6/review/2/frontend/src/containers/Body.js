...
const Body = () => {
  ...
  const handleLogicQuery = async () =>{
    const{
      data:{ data, message }
    } = await axios.get('/api/logic-query', {
      params:{
        queryLogic,
        queryName,
        querySubject
    ...
    if (!data) addErrorMessage(message);
    else addRegularMessage(...data);
  }
    ...
    <Wrapper>
      ...
      <Row>
        ...
        <StyledFormControl>
          <FormControl component="fieldset">
            <RadioGroup
              row
              value={queryLogic}
              onChange={handleChange(setQueryLogic)}
            >
              <FormControlLabel
                value="and"
                control={<Radio color="primary" />}
        ...
        <Button
          ...
          variant="contained"
          color="primary"
          disabled={!queryName || !querySubject}
          onClick={handleLogicQuery}
        >
          Query
        </Button>
      </Row>
